...
) : IDataSourceAsync<CustardMySqlEntity>
{
    ...
    public async Task<Result<List<CustardMySqlEntity>>> LoadAsync()
    {
        DateTime twentyTwelve = new(new DateOnly(2012, 1, 1), new TimeOnly(0), DateTimeKind.Utc);
        Result<List<CustardMySqlEntity>> found = await _repo.FindAsync(s => s.dateAdded >= twentyTwelve, true);
        return found;
    }

    public async Task<Result<List<CustardMySqlEntity>>> RefreshAsync()
    {
        DateTime lastMonth = DateTime.Now.AddDays(-30);
        Result<List<CustardMySqlEntity>> found = await _repo.FindAsync(s => s.dateAdded >= lastMonth, true);
        return found;
    }
}
